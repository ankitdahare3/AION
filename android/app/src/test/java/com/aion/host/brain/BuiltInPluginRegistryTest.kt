...
class BuiltInPluginRegistryTest {
    ...
    fun `the 8 named built-ins register successfully and are enabled per the owner's explicit HC-5 approval`() =
        runTest {
            ...
            val executor =
                DispatcherActionExecutor(ActionDispatcher(auditLogger, KillSwitch(auditLogger)), MlKitOcrEngine())
            ...
            for (id in ids) {
                assertTrue(
                    "$id should be enabled per the owner's explicit HC-5 approval",
                    registry.manager.isEnabled(id),
                )
                ...
                assertTrue(
                    ...
                    routed is PluginRouteResult.Rejected &&
                        (routed as PluginRouteResult.Rejected).reason.contains("no tool named"),
                )
            }
        }
